...
for(let i = 0; i < 60; i++){
  ...
  transform:rotate(${(i-1)*-6}deg);">${i+1}秒</div>`;
  second=second+div;
}
...
for(let i = 0; i < 60; i++){
  ...
  position:absolute;
  display:inline-block;
  transform:rotate(${(i-1)*-6}deg);">${i+1}分</div>`; 
  minute=minute+div; 
}
...
for(let i = 0; i < 24; i++){
  ...
  position:absolute;
  display:inline-block;
  transform:rotate(${i*-15}deg);">${i+1}时</div>`; 
  hour=hour+div; 
}
...
for(let i = 0; i < _days[_month]; i++){
  ...
  position:absolute;
  display:inline-block;
  transform:rotate(${i*-1*_deg}deg);">${i+1}日</div>`; 
  day=day+div; 
}
...
for(let i = 0; i < 12; i++){
  ...
  position:absolute;
  display:inline-block;
  transform:rotate(${i*-30}deg);">${i+1}月</div>`; 
  month=month+div; 
}
...
function transformBox(){
  ...
  document.querySelector('.second-box').style.
  transform=`rotate(${second360*360+(second-1)*6}deg)`,
  document.querySelector('.minute-box').style.
  transform=`rotate(${minute360*360+(minute-1)*6}deg)`;
  document.querySelector('.hour-box').style.
  transform=`rotate(${hour360*360+(hour-1)*15}deg)`;
  document.querySelector('.day-box').style.
  transform=`rotate(${day360*360+(day-1)*_deg}deg)`;
  document.querySelector('.month-box').style.
  transform=`rotate(${month*360+month*30}deg)`;
  document.querySelector('.year-box').innerHTML=year+'年';
  let nowDates=document.querySelectorAll('.now-date');
  nowDates.forEach((ele)=>{
    ele.classList='';
  });
  document.querySelector(`#second${second+1}`).
  classList='now-date';
  document.querySelector(`#minute${minute===0?'60':minute}`).
  classList='now-date';
  document.querySelector(`#hour${hour===0?'24':hour}`).
  classList='now-date';
  document.querySelector(`#day${day}`).
  classList='now-date';
  document.querySelector(`#month${month+1}`).
  classList='now-date';
